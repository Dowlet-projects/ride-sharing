...
func SetupRouter(db *sql.DB, cfg *config.Config) *mux.Router {
	...
	protected.HandleFunc("/passenger-profile", app.PassengerProfile).Methods("GET", "OPTIONS")
...
// PassengerProfile handles GET /passenger-profile
// @Summary Get passenger profile for passenger user
// @Description Retrieve passenger
// @Tags Announcement
// @Produce json
// @Security BearerAuth
// @Router /protected/passenger-profile [get]
func (h *App) PassengerProfile(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.DB.QueryRow("SELECT id, full_name, phone FROM passengers WHERE id = ? ", passengerID).Scan(&passenger.ID, &passenger.FullName, 
